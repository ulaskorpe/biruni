<template>
    <Head head-key="title" title="Form Güncelle" />
                            ...
                            <template #item="{element:item,index}">
                                <div :class="item.size">
                                    <Panel toggleable collapsed :header="item.itemLabel">
                                        ...
                                        <div v-if="['contentList'].includes(item.type)">
                                            <Divider/>
                                            <div class="row align-items-center">
                                                <label class="col-lg-6 fw-bold">İçerik Tipleri</label>
                                                <div class="col-lg-6">
                                                    <div class="d-flex flex-column gap-2 bg-light border p-3 rounded">
                                                        <MultiSelect v-model="item.rangeOptions.contentTypes" :options="$page.props.contentTypes" optionLabel="name" optionValue="id" placeholder="Lütfen seçiniz" :maxSelectedLabels="5" class="w-100"/>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
let inputTypes = ['text','number','email','textarea','password','select','multiselect','radio','checkbox','intTelInput','stateSelect','citySelect','range',
    'radioWithIcon',
    'checkboxWithIcon','contentList'];
...
const form = useForm({
    ...
    json_data:props.form.json_data ? props.form.json_data : [],
    sender_name:props.form.sender_name,
    to_email:props.form.to_email,
    subject:props.form.subject,
    button_text:props.form.button_text,
    success_type:props.form.success_type,
    success_message:props.form.success_message,
    success_uri:props.form.success_uri,
    success_script:props.form.success_script ? props.form.success_script : ``,
    is_modal:props.form.is_modal ?? false,
    modal_button_text:props.form.modal_button_text ?? "",
    cc_email:props.form.cc_email,
    bcc_email:props.form.bcc_email,
    email_content_id:props.form.email_content_id,
    kvkk_check:props.form.kvkk_check,
    privacy_check:props.form.privacy_check,
    membership_check:props.form.membership_check,
    step_count:props.form.step_count,
    column_count:props.form.column_count,
});

const update = () => {
    form.put(route('forms.update',props.form),{
        only:['form','flash','errors'],
        onSuccess: () => {
